...
class _SpectrumParent:
    ...
    def plot_writefile(self, high=None, filename="spec.jpg", **options):
        """Plots amplitude vs frequency.
...
class Wave:
    ...
    def make_spectrum(self, full=False):
        ...
        n = len(self.ys)
        d = 1 / self.framerate
        # np.fft.rfftfreq(fft length, sample interval time)
        if full:
    ...
    def plot_writefile(self, filename="sound.jpg"):
        """Plots the wave.

        If the ys are complex, plots the real part.

        """
        plt.plot(self.ts * 1, np.real(self.ys))
        plt.savefig(filename)
        plt.show()
